...
const CourseDetails = () => {
  ...
  const {allCourses, calculateRating, calculateNoOfLectures, calculateCourseDuration,
        calculateChapterTime} = useContext(AppCotext)
      ...
      <div className='max-w-xl z-10 text-gray-500'>
        ...
        <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
                 ...
                 <p>{courseData.enrolledStudents.length} {courseData.enrolledStudents.length > 1 ? 'students' : 'student' } </p>
                    ...
                    <p className='text-sm'>Course by <span className='text-blue-600 underline'>SkillSync</span></p>
                          ...
                          {courseData.courseContent.map((chapter, index)=> (
                            <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                             ...
                             <div className={`overflow-hidden transition-all duration-300 ${openSections[index] ? 'max-h-96' : 'max-h-0'}`}>
                              <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-tborder-gray-300'>
